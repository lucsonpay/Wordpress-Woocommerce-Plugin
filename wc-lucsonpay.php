<?php
/*
 Plugin Name: Lucsonpay Gateway for WooCommerce
 Plugin URI: http://www.lucsonpay.com/
 Description: LucsonPay PG WooCommerce integration
 Version: 1.0
 Author: Lucsonpay <support@lucsonpay.com>
 Author URI: http://www.lucsonpay.com
 */

 add_action('plugins_loaded', 'woocommerce_lucsonpay_init', 0);

/**
 * [woocommerce_lucsonpay_init description]
 * @return [type] [description]
 */
function woocommerce_lucsonpay_init()
{
    if (! class_exists('WC_Payment_Gateway')) {
        return;
    }
    class WC_Gateway_LucsonPay extends WC_Payment_Gateway
    {
        public function __construct()
        {
            global $woocommerce;
            $this->id           = 'lucsonpay';
            $this->method_title = __('LucsonPay Payment Gateway', 'woocommerce');
            $this->icon         = apply_filters('woocommerce_lucsonpay_icon', plugins_url().'/wc-lucsonpay/images/logo.png');
            ...
            // $this->pg_request_url       = 'https://merchant.lucsonpay.com/crm/jsp/paymentrequest';
            $this->pg_request_url       = 'https://uat.lucsonpay.com/crm/jsp/paymentrequest';

            // Actions
            add_action('init', array( $this, 'check_lucsonpay_response' ));
            add_action('woocommerce_api_wc_gateway_lucsonpay', array( $this, 'check_lucsonpay_response' ));
            add_action('woocommerce_receipt_lucsonpay', array(&$this, 'receipt_page'));
        ...
        {
            echo '<h3>LucsonPay Payment Gateway</h3>';
            echo '<p>LucsonPay Payment Gateway is a payment product which makes it extremely safe and easy to pay online.</p>';
        ...
        public function process_payment($order_id)
        {
            ...
            $this->return_url = trailingslashit(home_url()).'?wc-api=WC_Gateway_LucsonPay&order_id='.$order->id;
            $this->init_settings();

            require_once dirname(__FILE__).'/LPPGModule.php';
            $transaction_request = new LPPGModule();
        ...
        public function check_lucsonpay_response()
        {
        ...
        public function init_form_fields()
        {
            $this->form_fields = array(
                'enabled' => array(
                    ...
                    'type'    => 'checkbox', 
                    'label'   => __( 'Enable LucsonPay Gateway', 'woocommerce' ), 
                    'default' => 'yes'
                ), 
                'mode' => array(
                ...
                'title' => array(
                    ...
                    'default'     => __('LucsonPay Gateway', 'woocommerce'),
                    'desc_tip'    => true,
                ),
                'description' => array(
    ...
    function add_lucsonpay_gateway($methods)
    {
        $methods[] = 'WC_Gateway_LucsonPay';
        return $methods;
    }

    add_filter('woocommerce_payment_gateways', 'add_lucsonpay_gateway');
}
